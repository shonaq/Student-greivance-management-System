...
--
-- Host: 127.0.0.1
-- Generation Time: Feb 08, 2022 at 05:31 AM
...
UPDATE investigation, complaint SET investigation.priority='HIGH' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='sexual abuse');

ELSEIF (atype='drug abuse') THEN
UPDATE investigation, complaint SET investigation.priority='HIGH' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='drug abuse');

ELSEIF (atype='harassment') THEN
UPDATE investigation, complaint SET investigation.priority='HIGH' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='harassment');

ELSEIF (atype='ragging') THEN
UPDATE investigation, complaint SET investigation.priority='HIGH' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='ragging');

ELSEIF (atype='mental abuse') THEN
UPDATE investigation, complaint SET investigation.priority='MID' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='mental abuse');

ELSEIF (atype='physical abuse') THEN
UPDATE investigation, complaint SET investigation.priority='MID' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='physical abuse');

ELSEIF (atype='emotional abuse') THEN
UPDATE investigation, complaint SET investigation.priority='MID' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='emotional abuse');

ELSEIF (atype='financial abuse') THEN
UPDATE investigation, complaint SET investigation.priority='MID' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='financial abuse');

ELSEIF (atype='discrimination') THEN
UPDATE investigation, complaint SET investigation.priority='MID' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='discrimination');

ELSEIF (atype='other') THEN
UPDATE investigation, complaint SET investigation.priority='LOW' 
WHERE investigation.investigation_id IN(SELECT investigation.investigation_id FROM investigation, complaint 
WHERE investigation.complaint_id=complaint.complaint_id AND complaint.abuse_type='other');
...
(19, '1js19is124', '1js19me070', 'discrimination', '2022-02-08');
...
('1js19is124', 'shashank s p', 'male', 21, 0, 'ise', '3692587410', 'std1'),
...
(18, '1js19is124', 'INSERTED', '2022-02-08 09:37:59');
...
--
ALTER TABLE `complaint`
  MODIFY `complaint_id` int(255) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=20;

--
-- AUTO_INCREMENT for table `investigation`
--
ALTER TABLE `investigation`
  MODIFY `investigation_id` int(255) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=15;
